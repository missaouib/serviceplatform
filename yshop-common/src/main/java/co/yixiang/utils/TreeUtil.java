...
public class TreeUtil {
    ...
    public static List<DiseaseDTO> list2TreeConverter4Disease(List<DiseaseDTO> list, Long parentId) {
        List<DiseaseDTO> returnList = new ArrayList<>();

        for (DiseaseDTO res : list) {
            //判断对象是否为根节点

            if (res.getPid().equals(parentId)) {
                //该节点为根节点,开始递归

                recursionFn4Disease(list, res); //通过递归为节点设置childList
    ...
    public static void recursionFn4Disease(List<DiseaseDTO> list, DiseaseDTO t) {
        //只能获取当前t节点的子节点集,并不是所有子节点集
        List<DiseaseDTO> childsList = getChildList4Disease(list, t);
        //设置他的子集对象集
        t.setChildren(childsList);

        //迭代子集对象集
        for (DiseaseDTO nextChild : childsList) { //遍历完,则退出递归

            //判断子集对象是否还有子节点
            if (!CollectionUtils.isEmpty(childsList)) {
                //有下一个子节点,继续递归
                recursionFn4Disease(list, nextChild);
            }
        }
    }
    ...
    public static List<DiseaseDTO> getChildList4Disease(List<DiseaseDTO> list, DiseaseDTO t) {
        List<DiseaseDTO> childsList = new ArrayList<DiseaseDTO>();
        //遍历集合元素,如果元素的Parentid==指定元素的id,则说明是该元素的子节点
        for (DiseaseDTO t1 : list) {
