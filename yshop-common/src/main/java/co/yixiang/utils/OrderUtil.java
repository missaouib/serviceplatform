...
public class OrderUtil {
    ...
    public static String orderStatusStr(int paid, int status,
        ...
        if (paid == 0 && status == 0) {
            ...
            OrderStatusEnum orderStatusEnum = OrderStatusEnum.toType(status);
            if(orderStatusEnum != null) {
                statusName = orderStatusEnum.getDesc();
            }
        }
     ...
     * @return
     */
    public static int orderStatus(int paid, int status, int refund_status,int need_refund) {
        ...
        if (paid == 0 && status == 0 && refund_status == 0) {
            ...
            _status = 7;
        } else if (paid == 1 && need_refund == 1) {   // 审核不通过的需要退款
            _status = 3;
        } else {
            _status = status;
        }
    ...
    public static String payTypeName(String pay_type, int paid) {
        PayTypeEnum payTypeEnum = PayTypeEnum.toType(pay_type);
        if(payTypeEnum == null) {
            return "";
        } else{
            return PayTypeEnum.toType(pay_type).getDesc();
        }
    ...
    public static String generateOrderNoByUUId16() {
        int hashCodeV = (curDateTimeStr14()+ generateString(50)).hashCode();

        if(hashCodeV < 0) {//有可能是负数
            hashCodeV = - hashCodeV;
        }
        DecimalFormat format = new DecimalFormat("0000000000");
        return curDateStr("yyMMdd")+format.format(hashCodeV).substring(0,10);
    }
}
