...
public class DictDetailController extends BaseController {
    ...
    */
    ...
    @AnonymousAccess
    @PostMapping("/getList")
    @ApiOperation(value = "获取DictDetail列表",notes = "数据字典详情列表",response = DictDetailQueryVo.class)
    public ApiResult<List<DictDetail>> getDictDetailList(@Valid @RequestBody(required = false) DictDetailQueryParam dictDetailQueryParam) {
        List<DictDetail> list = dictDetailService.getDictDetailList(dictDetailQueryParam);
