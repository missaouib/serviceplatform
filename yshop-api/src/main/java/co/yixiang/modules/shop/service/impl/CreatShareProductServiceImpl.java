...
public class CreatShareProductServiceImpl implements CreatShareProductService {
    ...
    public  String creatProductPic(YxStoreProduct productDTO, String shareCode, String spreadPicName, String spreadPicPath, String apiUrl) throws IOException, FontFormatException {
        ...
        if(ObjectUtil.isNull(attachmentT)){
            ...
            try {
                String t = productDTO.getImage();
            //    String t1 = URLUtil.encode(t);
                URL url = new URL(t );
                priductUrl = ImageIO.read(url);
            } catch (IOException e) {
                e.printStackTrace();
            }
            ...
            String name =  productDTO.getCommonName();
            if(StrUtil.isNotBlank(productDTO.getStoreName())) {
                name = name + " ["+productDTO.getStoreName()+"]";
            }
            log.info("name========={}",name);
            int fontlenb = getWatermarkLength(name, g);
            //文字长度相对于图片宽度应该有多少行
            ...
            int fontlen = getWatermarkLength(StrUtil.emptyIfNull(productDTO.getStoreInfo()), g);
            //文字长度相对于图片宽度应该有多少行
            ...
            for(int i=0; i < StrUtil.emptyIfNull(productDTO.getStoreInfo()).length(); i++) {
                char tempChar = StrUtil.emptyIfNull(productDTO.getStoreInfo()).charAt(i);
...
//            g.drawString(productDTO.getStoreName(), 30, 1229);
            ...
            try {
                qrCode = ImageIO.read(new URL(URLUtil.encode(url)));
            } catch (IOException e) {
                e.printStackTrace();
            }
            // 绘制缩小后的图
            ...
            g.setColor(new Color(171,171,171));
            //绘制文字
            g.drawString("扫描或长按二维码", 515, 1260);
