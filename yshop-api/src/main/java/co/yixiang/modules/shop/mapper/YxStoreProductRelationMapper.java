...
public interface YxStoreProductRelationMapper extends BaseMapper<YxStoreProductRelation> {
    ...
    @Select("select B.id pid,A.category,B.store_name as storeName,B.price," +
            "B.ot_price as otPrice,B.sales,B.image,B.is_del as isDel,B.is_show as isShow,B.common_name,B.spec,B.manufacturer,A.project_code as projectCode" +
            " from yx_store_product_relation A left join yx_store_product B " +
            "on A.product_id = B.id where A.uid=#{uid} and A.project_code = #{projectCode}")
    List<YxStoreProductRelationQueryVo> selectList(Page page,@Param("uid") int uid,@Param("projectCode") String projectCode);
    ...
    @Select("select B.id pid,A.category,B.store_name as storeName,B.price," +
            "B.ot_price as otPrice,B.sales,B.image,B.is_del as isDel,B.is_show as isShow,B.common_name,B.spec,B.manufacturer,A.project_code as projectCode" +
            " from yx_store_product_relation A left join yx_store_product B " +
            "on A.product_id = B.id where A.uid=#{uid} and A.project_code in ('', #{projectCode})")
    List<YxStoreProductRelationQueryVo> selectList2(Page page,@Param("uid") int uid,@Param("projectCode") String projectCode);
