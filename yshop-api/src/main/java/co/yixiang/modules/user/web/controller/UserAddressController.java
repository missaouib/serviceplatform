...
public class UserAddressController extends BaseController {
    ...
    */
    ...
    public ApiResult<Map<String,Object>> addYxUserAddress(@Valid @RequestBody AddressParam param){
        ...
        userAddress.setType(param.getType());
        ...
        if(param.getIs_default().equals("true")){
            ...
            userAddressService.update(address,
                    new QueryWrapper<YxUserAddress>().lambda().eq(YxUserAddress::getUid,uid).eq(YxUserAddress::getType,param.getType()));
            YxUserAddress userAddress2 = new YxUserAddress();
            userAddress2.setIsDefault(1);
            userAddress2.setId(userAddress.getId());
            boolean flag = userAddressService.updateById(userAddress2);
        }
     ...
     * 添加或修改地址
     */
    @Log(value = "新增站点地址",type = 1)
    @PostMapping("/address/add")
    @ApiOperation(value = "添加或修改地址",notes = "添加或修改地址",response = ApiResult.class)
    public ApiResult<Map<String,Object>> addYxUserAddress2(@Valid @RequestBody AddressParam param){
        int uid = SecurityUtils.getUserId().intValue();
        ...
        param.setType(0);
        userAddress.setType(param.getType());
        LambdaQueryWrapper<YxUserAddress> yxUserAddressLambdaQueryWrapper = new LambdaQueryWrapper<>();
        yxUserAddressLambdaQueryWrapper.eq(YxUserAddress::getUid,uid);
        yxUserAddressLambdaQueryWrapper.eq(YxUserAddress::getDetail,param.getDetail());
        yxUserAddressLambdaQueryWrapper.eq(YxUserAddress::getPhone,param.getPhone());
        yxUserAddressLambdaQueryWrapper.eq(YxUserAddress::getRealName,param.getReal_name());

        YxUserAddress yxUserAddress = userAddressService.getOne(yxUserAddressLambdaQueryWrapper,false);

        if(yxUserAddress == null) {
            userAddress.setAddTime(OrderUtil.getSecondTimestampTwo());
            userAddressService.save(userAddress);
        } else {
            userAddress = yxUserAddress;
        }
    ...
    public ApiResult<Boolean> setDefault(@Valid @RequestBody IdParam idParam){
        ...
        address.setIsDefault(0);
        userAddressService.update(address,
                new QueryWrapper<YxUserAddress>().lambda().eq(YxUserAddress::getUid,uid).eq(YxUserAddress::getType,0));
        YxUserAddress userAddress = new YxUserAddress();
        userAddress.setIsDefault(1);
