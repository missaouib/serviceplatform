...
public interface YxStoreOrderService extends BaseService<YxStoreOrder> {
    ...
    List<YxStoreOrderQueryVo> orderList(int uid,int type,int page,int limit,YxStoreOrderQueryParam queryParam);
    ...
    YxStoreOrderQueryVo handleOrder4Store(YxStoreOrderQueryVo order);
    ...
    WxPayMpOrderResult wxAppPay4ThirdPartyOpenid(String orderId,String openId) throws WxPayException;
    ...
    YxStoreOrder createOrder4Store(int uid, String key, OrderParam param);

    YxStoreOrder createOrder4External(int uid, String key, OrderExternalParam param);

    YxStoreOrder createOrder4Project(int uid, String key, OrderParam param);
    ...
    List<YxStoreOrderQueryVo> yiyaobaoOrderList(int uid,YxStoreOrderQueryParam queryParam);

    YxStoreOrderQueryVo queryOrderDetail(String orderId);

    void dualCSOrder(List<String> orderNoList);

    void updatePayVoucher(OrderParam param);

    String getHpUrl(String orderId);

    void sendOrder2YiyaobaoCloud(String orderId);

    void sendOrder2YiyaobaoStore(String orderId);

    void sendOrder2YiyaobaoCloudCancel(String orderId);

    YxStoreOrder addOrder4Project(Order4ProjectParam order4ProjectParam);


    ComputeDTO computedOrder4Project(int uid, String projectCode, BigDecimal totalPrice, String addressId,Integer expressTemplateId);

    String getRocheHospitalName(int uid);

    YxStoreOrder findByUidAndOriginalOrderNo(int uid, String orderNumber);

    String aliPayH5Pay(String orderId);

    String alipayTradeAppPay(String orderId,String userId);

    void updateOrderById(YxStoreOrder storeOrder);

    List<PayDTO> queryPayType(String projectCode,String contextParam);

    WxPayMpOrderResult newWxAppPay(String orderId) throws WxPayException;

    WxPayMwebOrderResult newWxH5Pay(String orderId) throws WxPayException;
