...
public interface YxStoreOrderMapper extends BaseMapper<YxStoreOrder> {
    ...
    @Select("select roche_hospital_name from yx_store_order where  roche_hospital_name is not null and roche_hospital_name !='' and uid=#{uid} order by add_time desc limit 1")
    String getRocheHospitalName(int uid);

    YxStoreOrder findByUidAndOriginalOrderNo(@Param("uid")int uid, @Param("orderNumber")String orderNumber);
}
