...
import co.yixiang.modules.security.rest.param.TokenParam;
...
public class AuthController {
    ...
    public ApiResult<Map<String, String>> login(@Validated @RequestBody AuthUser authUser,
        ...
        authInfo.put("expires_time", expiresTimeStr);
    ...
    public ApiResult<Object> authLogin(@RequestParam(value = "code") String code,
                                       ...
                                       HttpServletRequest request) {
        log.info("/wechat/auth,wechatName={}",wechatName);
        /**
         * 公众号与小程序打通说明：
         ...
         * 3、如果你以前已经有数据请自行处理
         */
        return ApiResult.ok(userService.authLogin(code,spread,request,wechatName));
    }
     ...
     * 微信公众号授权
     */
    @GetMapping("/wechat/auth2")
    @ApiOperation(value = "微信公众号授权", notes = "微信公众号授权")
    public ApiResult<Object> authLogin2(@RequestParam(value = "code") String code,
                                       @RequestParam(value = "wechatName",required=false,defaultValue = "") String wechatName,
                                       HttpServletRequest request) {
        log.info("/wechat/auth2,wechatName={}",wechatName);
        /**
         * 公众号与小程序打通说明：
         * 1、打通方式以UnionId方式，需要去注册微信开放平台
         * 2、目前登陆授权打通方式适用于新项目（也就是你yx_user、yx_wechat_user都是空的）
         * 3、如果你以前已经有数据请自行处理
         */
        return ApiResult.ok(userService.authLogin2(code,request,wechatName));
    }
     ...
     * 小程序授权接口
     */
    @PostMapping("/wxapp/auth2")
    @ApiOperation(value = "小程序授权", notes = "小程序授权")
    public ApiResult<Object> auth(@Validated @RequestBody LoginParam loginParam,
                                   HttpServletRequest request) {
        /**
         * 公众号与小程序打通说明：
         * 1、打通方式以UnionId方式，需要去注册微信开放平台
         ...
         * 3、如果你以前已经有数据请自行处理
         */
        log.info("小程序授权{}",loginParam);
        return ApiResult.ok(userService.wxappAuth2(loginParam,request)) ;
    ...
    public ApiResult<String> verify(@Validated @RequestBody VerityParam param) {
       ...
       /* if (ObjectUtil.isNotNull(redisUtils.get(codeKey))) {
        ...
        String remindmessage = "【益药】验证码：%s。有效时间为10分钟，请妥善保管";
        remindmessage = String.format(remindmessage, code);
        co.yixiang.tools.service.dto.SmsResult smsResult = smsService.sendTeddy("",remindmessage,param.getPhone());
        if(smsResult != null && StrUtil.isNotBlank(smsResult.getError())) {
            return ApiResult.ok("发送成功，请注意查收");
        } else {
            redisUtils.del(codeKey);
            if(smsResult != null) {
                return ApiResult.ok("发送失败："+smsResult.getError());
            } else {
                return ApiResult.ok("发送失败");
            }
    ...
    @ApiOperation(value = "验证token有效性", notes = "验证token有效性")
    @AnonymousAccess
    @PostMapping(value = "/validateToken")
    public ApiResult<Object> validateToken(@RequestBody TokenParam tokenParam) {
        String token = tokenParam.getToken();
        if (org.apache.commons.lang3.StringUtils.isEmpty(token)){
            return ApiResult.ok(false);
        }
        log.info("token 验证====== {}",token);
        ...
        Date expiresTime = tokenProvider.getExpirationDateFromToken(token);
        String expiresTimeStr = DateUtil.formatDateTime(expiresTime);
        log.info("token的过期时间:{}",expiresTimeStr);
