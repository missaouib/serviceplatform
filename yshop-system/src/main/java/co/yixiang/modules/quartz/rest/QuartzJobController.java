...
public class QuartzJobController {
    ...
    public ResponseEntity<Object> updateIsPause(@PathVariable Long id){
        //if(StrUtil.isNotEmpty("22")) throw new BadRequestException("演示环境禁止操作");
        quartzJobService.updateIsPause(quartzJobService.getOne(new QueryWrapper<QuartzJob>().eq("id",id)));
        return new ResponseEntity<>(HttpStatus.NO_CONTENT);
    }
    ...
    @GetMapping(value = "/{id}")
    @PreAuthorize("@el.check('admin','timing:edit')")
    public ResponseEntity<Object> updateIsPauseGet(@PathVariable Long id){
        //if(StrUtil.isNotEmpty("22")) throw new BadRequestException("演示环境禁止操作");
        quartzJobService.updateIsPause(quartzJobService.getOne(new QueryWrapper<QuartzJob>().eq("id",id)));
        return new ResponseEntity<>(HttpStatus.NO_CONTENT);
    }
    ...
    @PreAuthorize("@el.check('admin','timing:edit')")
    public ResponseEntity<Object> executionGet(@PathVariable Long id){
        //if(StrUtil.isNotEmpty("22")) throw new BadRequestException("演示环境禁止操作");
        quartzJobService.execution(quartzJobService.getOne(new QueryWrapper<QuartzJob>().eq("id",id)));
        return new ResponseEntity<>(HttpStatus.NO_CONTENT);
    }
