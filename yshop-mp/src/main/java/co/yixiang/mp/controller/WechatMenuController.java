...
public class WechatMenuController {
    ...
    public ResponseEntity create( @RequestBody String jsonStr){
        ...
        Object wechatName_object = jsonObject.get("wechatName");
        String wechatName = "";
        if( ObjectUtil.isNotNull(wechatName_object)) {
            wechatName = wechatName_object.toString();
        }

        String wechat_menus_key = "wechat_menus" + wechatName;
        Boolean isExist = YxWechatMenuService.isExist(wechat_menus_key);

        WxMpService wxService = WxMpConfiguration.getWxMpService(wechatName);
        if(isExist){
            YxWechatMenu.setKey(wechat_menus_key);
