...
import co.yixiang.mp.exam.dao.*;
import co.yixiang.mp.exam.domain.*;
import co.yixiang.mp.exam.service.StudentHomeService;
import co.yixiang.mp.exam.util.FindContentWithImage;
import co.yixiang.mp.exam.util.MultipleAnswersUtil;
...
public class StudentHomeServiceImpl implements StudentHomeService {
    ...
    public Map<String, Integer> getPaperQueNumRandomByPaperId(Integer paperId, String sno) {
        ...
        int totalNum = paperQueMapper.countByExampleRandom(example);
        criteria.andQueTypeEqualTo(1);
        criteria.andSnoEqualTo(sno);
        int singleNum = paperQueMapper.countByExampleRandom(example);
        ...
        result.put("totalNum",totalNum);
        result.put("singleNum",singleNum);
        result.put("multipleNum",0);
        result.put("judgeNum",0);
        result.put("fillNum",0);
    ...
    public List<Map<String, Object>> getSingleQueListByPaperId(Integer paperId) {
        ...
        for (BankSingleChoiceQue bankSingleChoiceQue : singleChoiceQueList){
            ...
            map.put("answer",bankSingleChoiceQue.getSingleAnswer());
...
//        System.out.println(singleChoiceQueList.get(0).getPaperQue().getQueId());
    ...
    public List<Map<String, Object>> getSingleQueListRadomByPaperId(Integer paperId,String sno) {
        ...
        List<BankSingleChoiceQue> singleChoiceQueList = bankSingleChoiceQueMapper.getSingleQueListRadomByPaperId(paperId,sno);
        for (BankSingleChoiceQue bankSingleChoiceQue : singleChoiceQueList){
            Map<String, Object> map = new HashMap<>();
...
//        System.out.println(singleChoiceQueList.get(0).getPaperQue().getQueId());
    ...
    public List<Map<String, Object>> getMultipleQueListByPaperId(Integer paperId) {
        ...
        for (BankMultipleChoiceQue bankMultipleChoiceQue : multipleChoiceQueList){
            ...
            map.put("answer",bankMultipleChoiceQue.getMultipleAnswer());
            map.put("onoff",false);
            List<Map<String, String>> optionsList = new ArrayList<>();
    ...
    public List<Map<String, Object>> getJudgeQueListByPaperId(Integer paperId) {
        ...
        for (BankJudgeQue bankJudgeQue : judgeQueList){
            ...
            map.put("answer",bankJudgeQue.getJudgeAnswer());
            List<Map<String, String>> optionsList = new ArrayList<>();
    ...
    public List<Map<String, Object>> getFillQueListByPaperId(Integer paperId) {
        ...
        for (BankFillQue bankFillQue : fillQueList){
            ...
            map.put("answer",bankFillQue.getFillAnswer());
            map.put("onoff",false);
            listResult.add(map);
        }
...
//                    System.out.println("dd" + fillAnswers.get(i));
    ...
    */
    ...
    public int savePaperHistory(String sno, Integer paperId, String json) {
        StudentPaperHistory studentPaperHistory = studentPaperScoreMapper.getStudentPaperHistory(sno,paperId);
        int result = 0;
        if(studentPaperHistory == null) {
            studentPaperHistory = new StudentPaperHistory();
            studentPaperHistory.setSno(sno);
            studentPaperHistory.setPaperId(paperId);
            studentPaperHistory.setHistory(json);
            result = studentPaperScoreMapper.insertPaperHistory(studentPaperHistory);
        } else {
            studentPaperHistory.setHistory(json);
            result = studentPaperScoreMapper.updatePaperHistoryById(studentPaperHistory);
        }
        return result;
    }

    @Override
    public StudentPaperHistory getStudentPaperHistory(String sno, Integer paperId) {
        return studentPaperScoreMapper.getStudentPaperHistory(sno,paperId);
    }

    @Override
    public void deleteStudentPaperHistory(String sno, Integer paperId) {
        studentPaperScoreMapper.deleteStudentPaperHistory(sno,paperId);
    }
}
